...
const LexerLevel = require("./LexerLevel2")
...
class SyntaxLevel2 {
    ...
    functionStatement() {
        ...
        if(nextToken && nextToken.type === TokenEnum.type.FUNCTION) {
            ...
            if(nextToken && nextToken.type === TokenEnum.type.Identifier) {
                let id =  new AstNode(TokenEnum.type.Identifier, nextToken.value)
                this.lexerLevel.tokenRead()
                node.addId(id)
                nextToken = this.lexerLevel.tokenPeek()
    ...
    functionBodyBlock() {
        let node = null;
        let nextToken = this.lexerLevel.tokenPeek()
        console.log("function body===0", nextToken)
        if(nextToken && nextToken.type === TokenEnum.type.LeftBrace) {
            ...
            if(nextToken && nextToken.type === TokenEnum.type.RightBrace) {
                this.lexerLevel.tokenRead();
            }
        }
        return node;
    ...
    ifStatement() {
        ...
        let nextToken = this.lexerLevel.tokenPeek()
        // 预判进入If语句
        if(nextToken && nextToken.type === TokenEnum.type.IF) {
            this.lexerLevel.tokenRead();
            // 构建一个if Ast
    ...
    returnStatement() {
        ...
        let nextToken = this.lexerLevel.tokenPeek()
        // 判断出Return
        if(nextToken && nextToken.type === TokenEnum.type.RETURN) {
            // 构建一个return Ast node
            node = new ReturnStatementAstNode(TokenEnum.type.RETURN, "return")
            this.lexerLevel.tokenRead()
            // 判断后面的表达式  todo 换行判断
    ...
    bitwiseShift() {
        ...
        if(child ) {
            while(true) {
                let nextToken = this.lexerLevel.tokenPeek()
                if(nextToken && (nextToken.type === TokenEnum.type.LeftShirt || nextToken.type === TokenEnum.type.RightShirt || nextToken.type === TokenEnum.type.UnsignedRightShift)) {
                    nextToken = this.lexerLevel.tokenRead();
    ...
    additive() {
        ...
        if(child ) {
            while(true) {
                let nextToken = this.lexerLevel.tokenPeek()
                if(nextToken && (nextToken.type === TokenEnum.type.Addition || nextToken.type === TokenEnum.type.Subtraction)) {
                    nextToken = this.lexerLevel.tokenRead();
    ...
    multiplicative() {
        ...
        if(child) {
            while(true) {
                let nextToken = this.lexerLevel.tokenPeek()
                if(nextToken && (nextToken.type === TokenEnum.type.Multiplication || nextToken.type === TokenEnum.type.Division)) {
                    nextToken = this.lexerLevel.tokenRead();
                ...
                while(true) {
                    ...
                    if(child2) {
                        ...
                        if(nextToken && (nextToken.type === TokenEnum.type.LeftBracket || nextToken.type === TokenEnum.type.DOT)) {
                            node = new MemberAstNode("Member", '', nextToken.type === TokenEnum.type.LeftBracket?true: false);
                            ...
                            nextToken = this.lexerLevel.tokenPeek()
                            if(nextToken && nextToken.type === TokenEnum.type.RightBracket) {
                                this.lexerLevel.tokenRead()
                                nextToken = this.lexerLevel.tokenPeek()
                            }
                            child = node;
    ...
    newExpression() {
        ...
        if(nextToken && nextToken.type === TokenEnum.type.NEW) {
            ...
            // TODO  非new的情况下, 只判断memberExpression , 且不能带参数
            let pos = this.lexerLevel.tokenSignPos()
            node = this.memberExpression();
            nextToken = this.lexerLevel.tokenPeek()
            if(nextToken && nextToken.type === TokenEnum.type.LeftParen) {
        ...
        // if(!child && nextToken) {
        //     if(nextToken.type === TokenEnum.type.NEW) {
        //         this.lexerLevel.tokenRead()
    ...
    primary() {
        ...
        if(nextToken) {
            ...
            if(nextToken.type === TokenEnum.type.THIS) {
                nextToken = this.lexerLevel.tokenRead();
                node = new PrimaryAstNode(TokenEnum.type.THIS, nextToken.value);
            } else if(nextToken.type === TokenEnum.type.Identifier) {
                nextToken = this.lexerLevel.tokenRead();
                node = new PrimaryAstNode(TokenEnum.type.Identifier, nextToken.value);
            } else if(nextToken.type === TokenEnum.type.NumericLiteral || nextToken.type === TokenEnum.type.StringLiteral || nextToken.type === TokenEnum.type.BooleanLiteral) {
                nextToken = this.lexerLevel.tokenRead();
                let v = nextToken.value
                if(v && nextToken.type === TokenEnum.type.StringLiteral) {
                    v = v.replace(/\'|\"/g, "");
                }
                node = new PrimaryAstNode(nextToken.type, v);
            } else if(nextToken.type === TokenEnum.type.LeftParen) {
                // v0.0.2 新增支持( )简单处理
                //
                nextToken = this.lexerLevel.tokenRead();
                // 检测下一个表达式
                ...
                if(node) {
                    nextToken = this.lexerLevel.tokenPeek();
                    if(nextToken.type === TokenEnum.type.RightParen) {
                        this.lexerLevel.tokenRead()
                    } else {
